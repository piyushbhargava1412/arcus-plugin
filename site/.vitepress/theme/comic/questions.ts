...
export const QUIZ: QuizQuestion[] = [
  ...
  { d: 'easy', q: 'What phrase starts gated mode by default?',
    o: ['arcus <STORY>', 'forge <STORY>', 'plan <STORY> or arcus <STORY>', 'sync context'],
    a: 2, w: 'Gated is Lucie’s default — `plan <STORY>` or `arcus <STORY>` trigger gated mode. `arcus <STORY> --intelligent` triggers intelligent mode, or `forge <STORY>`/`afk <STORY>`/`run afk on <STORY>`/`arcus <STORY> --afk` for AFK mode.' },
  ...
  { d: 'easy', q: 'What makes a skill different from an agent, at the simplest level?',
    ...
    a: 1, w: 'Skills live at plugins/arcus/skills/ and are called by their bare name (repo-agentifier); agents live flat under agents/ and are only ever dispatched as isolated subagents, never by you.' },
  ...
  { d: 'medium', q: 'ARCUS bundles a SessionStart hook that runs bootstrap.sh. Why does it re-stage the toolbox at the start of every run anyway?',
    o: ['To pick up new stories', 'Because .arcus/bin is a copy that never expires, regardless of whether a hook fired', 'To reset the checkpoint', 'It does not — the hook is enough'],
    a: 1, w: 'Both Claude Code and Copilot CLI do fire the bundled hook — Copilot CLI runs its command from the plugin\'s own install directory rather than the session\'s, so bootstrap.sh reads the real cwd off the hook\'s stdin payload and cd\'s there first. Either way, .arcus/bin is a snapshot with no expiry, so a repo bootstrapped once would keep serving those same scripts forever. locate.sh re-stages from the newest install on every entry point, so ARCUS never depends on any hook firing at all.' },
  ...
  { d: 'medium', q: 'In AFK mode, what happens to Angelina’s Open Questions list?',
    ...
    a: 1, w: 'The list is written identically in all three modes. Only the orchestrator differs: gated and intelligent show it to you, AFK ignores it — which is why an AFK spec is never less resolved, just less confirmed.' },
  ...
  { d: 'hard', q: 'Can Lucie change speed mid-story — switch between gated, intelligent, and AFK partway through a run?',
    ...
    a: 2, w: 'All three modes are the same Lucie underneath, but the mode choice is locked in from the start; you cannot switch mid-run.' },
  { d: 'hard', q: 'How many of ARCUS’s roughly 26 jobs are teammates you can call directly, versus ones only looped in by name?',
    o: ['All 26 are callable directly', 'None are callable directly', '8 are directly callable (skills); 18 are only looped in by name (agents)', '18 are directly callable; 8 are looped in by name'],
    a: 2, w: 'The skill/agent split is exactly this: 8 of 26 jobs are user-and-model invocable; the other 18 are dispatched by name only.' },
]

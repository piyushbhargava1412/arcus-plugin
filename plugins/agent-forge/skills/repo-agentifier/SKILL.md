...
- **Model Strategy**: Resolve model tier via the `agent-forge:model-strategy` skill for each subagent.
...
The orchestrator MUST be invoked from the root of a git repository with the `agent-forge` plugin enabled (or its skills available in the workspace's skills directory).
...
- If the skills folder cannot be located: Report `[ERROR] Agent Forge skills not found. Is the agent-forge plugin enabled?` and STOP.

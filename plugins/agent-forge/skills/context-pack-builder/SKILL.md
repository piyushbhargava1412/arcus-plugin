...
- Run the helper script to identify relevant flows without reading every file:
  `python3 "$AFORGE_HOME/skills/context-pack-builder/scripts/match_flows.py" .`
...
- **Matching Script**: `./scripts/match_flows.py` (resolve via `$AFORGE_HOME` for execution)

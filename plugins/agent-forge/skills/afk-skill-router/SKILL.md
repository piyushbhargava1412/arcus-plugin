...
description: >
  Agent Forge orchestrator meta-skill. Drives a human-gated SDLC pipeline
...
# Agent Forge Skill Router (Orchestrator Meta-Skill)
...
Resolve the mode once at entry and persist it: `.aforge/bin/checkpoint.sh set-mode <STORY_ID> <gated|afk>`.
...
in-progress story exists under `.aforge/specs/`; otherwise ask which story.
...
(`.aforge/bin/checkpoint.sh set-status <STORY_ID> <next_entry_key> awaiting_handoff`) and emit a
...
(read `AFORGE_HOME` from `.aforge/env`).
...
| `.aforge/bin/checkpoint.sh <action> <story-id> [args]` | Manage state | init / read / complete / set-status / reopen / set-mode |
...
1. **Extract Story ID**: Run `.aforge/bin/extract_story_id.sh <STORY_FILE>` and capture `STORY_ID`.
   - If the script is missing, derive the ID from the filename (strip path and `.md`).
2. **Check checkpoint**: Run `.aforge/bin/checkpoint.sh read <STORY_ID>`. If it exists, jump to the
   **Resumption Protocol** instead of re-initialising.
3. **Create branch + workspace**: Run `.aforge/bin/branch.sh <STORY_ID>`. Capture `BRANCH_NAME` and
   `BASE_BRANCH` from its output.
4. **Initialize checkpoint**: Run `.aforge/bin/checkpoint.sh init <STORY_ID> <BRANCH_NAME> <BASE_BRANCH> <MODE>`
   where `<MODE>` is `gated` or `afk` resolved at entry.
5. **Mark init complete**: `.aforge/bin/checkpoint.sh complete <STORY_ID> init`.
...
1. **Build context pack** — Dispatch a subagent:
   ...
   - Verify the file exists, then `.aforge/bin/checkpoint.sh complete <STORY_ID> context_pack`.
2. **Finalize spec** — How you run this depends on mode:
   - **In gated mode**: Read and follow the `agent-forge:spec-finalizer` skill **in the main thread**
     ...
     to `.aforge/specs/<STORY_ID>/clarifications.md`.
   - **In afk mode**: there is no dialogue, so dispatch it as a one-shot subagent:
     - **Prompt**: "Read and follow the `agent-forge:spec-finalizer` skill in one-shot mode. Story ID:
       `<STORY_ID>`. Auto-resolve every ambiguity and produce ONLY
       `.aforge/specs/<STORY_ID>/assumptions.md`. Do NOT create `blueprint.md`, a task list, or any
     ...
     - **Model**: Resolve complexity `medium` via the `agent-forge:model-strategy` skill.
   - In both modes, the skill produces only `.aforge/specs/<STORY_ID>/assumptions.md` (plus
   ...
   - Verify `assumptions.md` exists, then `.aforge/bin/checkpoint.sh complete <STORY_ID> spec_finalizer`.
3. **Create implementation plan** — Dispatch a subagent:
   ...
   - Verify the file exists, then `.aforge/bin/checkpoint.sh complete <STORY_ID> blueprint`.
...
1. **Compile test spec** — Dispatch a subagent:
   ...
   - **Model**: Resolve complexity `medium` via the `agent-forge:model-strategy` skill.
   - Verify the file exists, then `.aforge/bin/checkpoint.sh complete <STORY_ID> test_plan`.
...
1. **Read the dispatch protocol**: Read and follow the `agent-forge:subagent-task-dispatcher` skill.
2. **Read the model strategy**: Load `agent-forge:model-strategy` for complexity→model resolution.
3. **Parse tasks** from `.aforge/specs/<STORY_ID>/blueprint.md` (each `### Task N:` heading). If a
...
4. **For each task** (in order), and only those not already `complete`, follow the Subagent Task
   ...
   - Mark in-progress: `.aforge/bin/checkpoint.sh set-status <STORY_ID> task_<N> in_progress`.
   - On success: `.aforge/bin/checkpoint.sh complete <STORY_ID> task_<N>`.
...
1. **Run the review** — Dispatch the review coordinator:
   - **Prompt**: "Read and follow the `agent-forge:code-reviewer` skill. Story ID: `<STORY_ID>`. Review the full branch diff against base. Produce `.aforge/specs/<STORY_ID>/review.md` and end your reply with `VERDICT: approved | changes_requested`."
   - **Description**: "Review: code-reviewer"
   - **Model**: Resolve complexity `heavy` via the `agent-forge:model-strategy` skill.
   - Verify `review.md` exists. Capture the verdict and counts (`critical`, `warning`, `suggestion`).
   - `.aforge/bin/checkpoint.sh complete <STORY_ID> code_review`.
...
3. **Mark complete**: `.aforge/bin/checkpoint.sh complete <STORY_ID> closure`.
...
1. `.aforge/bin/checkpoint.sh reopen <STORY_ID> code_review` (sets `needs_rework`, bumps `review_round`).
2. Read `.aforge/specs/<STORY_ID>/review.md`. Convert each **critical** and **warning** finding into a
   ...
   `.aforge/bin/checkpoint.sh set-status <STORY_ID> task_<N> pending`.
...
1. Read it with `.aforge/bin/checkpoint.sh read <STORY_ID>` and read the persisted `mode`.

...
name: code-reviewer
description: >
  Holistic code-review coordinator for the Agent Forge Code Review stage. Reviews the
...
- `.aforge/specs/<STORY_ID>/blueprint.md` — what was supposed to be built
- `.aforge/specs/<STORY_ID>/assumptions.md` — the agreed decisions/scope
- `.aforge/specs/<STORY_ID>/test-plan.md` — expected test coverage
- `.aforge/specs/<STORY_ID>/context-pack.md` — architecture + repo patterns
- `.github/copilot-instructions.md` — repo conventions (if present)

## Output

- `.aforge/specs/<STORY_ID>/review.md` — the consolidated, severity-tagged review report
- A return message ending with exactly one line: `VERDICT: approved | changes_requested`

## Severity Taxonomy (canonical for all Agent Forge reviewers)
...
1. Read `base_branch` from `.aforge/specs/<STORY_ID>/session-checkpoint.json`.
...
`agent-forge:model-strategy` skill.

| Reviewer | Skill | Complexity | Scope |
|----------|-------|------------|-------|
| Spec compliance | `agent-forge:spec-compliance-reviewer` (holistic mode) | medium | Whole diff vs. blueprint DoD + assumptions |
| Code quality | `agent-forge:code-quality-reviewer` (holistic mode) | medium | Whole diff vs. repo patterns |
| Security | `agent-forge:security-reviewer` | medium | Whole diff |
| Performance | `agent-forge:performance-reviewer` | medium | Whole diff |
...
Write `.aforge/specs/<STORY_ID>/review.md` with:

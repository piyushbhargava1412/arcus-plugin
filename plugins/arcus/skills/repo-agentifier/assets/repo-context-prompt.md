...
1. Read and follow the `arcus:repository-context-builder` skill (it references its own templates and output specification internally).
...
- Ingest **all non-ignored relevant evidence** — not just source code, but infrastructure, scripts,
  ...
  deployment manifests. Honor root + nested `.gitignore` and any `.contextignore`/`.arcus-ignore`.

...
- Follow the guidelines in `${skills_folder}/implementation-planner/references/task-decomposition.md`.
- **Constraint**: Each task must be "atomic"—focused on a single logical change and including its own validation (tests).
- **Complexity Classification**: For each task, assess its difficulty and assign a `complexity` level (`heavy`, `medium`, or `light`). Use the guardrail heuristics in `${skills_folder}/shared/model-strategy.md` (Classification Guardrails section). Do NOT use model names — only difficulty levels.
...
- Use `${skills_folder}/implementation-planner/assets/blueprint-template.md` to structure the final plan.
- Write the output to `.aforge/specs/[STORY-ID]/blueprint.md`.
...
## Resources
- **Blueprint Template**: `${skills_folder}/implementation-planner/assets/blueprint-template.md`
- **Task Decomposition Guide**: `${skills_folder}/implementation-planner/references/task-decomposition.md`

...
Call these via shell for deterministic operations. They are installed at `.github/scripts/` in the target repo.

| Script | Usage | Purpose |
|--------|-------|---------|
| `.github/scripts/extract_story_id.sh <story.md>` | Outputs `STORY_ID: xxx` | Extract story identifier |
| `.github/scripts/branch.sh <story-id>` | Creates branch + workspace | Git isolation + scaffold |
| `.github/scripts/commit.sh <story-id> <message>` | Stages + commits | Conventional commit |
| `.github/scripts/pr.sh <story-id>` | Push + create PR | Closure |
| `.github/scripts/checkpoint.sh <action> <story-id> [stage]` | Manage state | read/init/complete |

## Execution Pipeline

### Stage 0: Init

1. **Extract Story ID**: Run `.github/scripts/extract_story_id.sh <STORY_FILE>` and capture the STORY_ID.
   - If the script is not found, extract the ID from the filename (strip `.md` extension and path).
2. **Check for existing checkpoint**: Run `.github/scripts/checkpoint.sh read <STORY_ID>`.
   - If checkpoint exists with completed stages, skip to the first incomplete stage.
3. **Create branch + workspace**: Run `.github/scripts/branch.sh <STORY_ID>`.
...
4. **Initialize checkpoint**: Run `.github/scripts/checkpoint.sh init <STORY_ID> <BRANCH_NAME> <BASE_BRANCH>`.
   - Pass the branch name and base branch captured from step 3. This ensures `pr.sh` targets the correct base branch.
5. **Mark init complete**: Run `.github/scripts/checkpoint.sh complete <STORY_ID> init`.
6. **Output**: `[AFK] Story: <STORY_ID>`

### Stage 1: Architect

1. **Build context pack** — Dispatch a subagent:
   - **Prompt**: "Read and follow `.github/skills/context-pack-builder/SKILL.md`. Story ID: `<STORY_ID>`. Produce `.aforge/specs/<STORY_ID>/context-pack.md`."
   ...
   - Run `.github/scripts/checkpoint.sh complete <STORY_ID> context_pack`.
2. **Finalize spec** — Dispatch a subagent:
   - **Prompt**: "Read and follow `.github/skills/spec-finalizer/SKILL.md`. Story ID: `<STORY_ID>`. Produce `.aforge/specs/<STORY_ID>/assumptions.md`."
   - **Description**: "Architect: spec-finalizer"
   - **Model**: Resolve complexity `heavy` via `.github/skills/shared/model-strategy.md`
   - Verify output file exists after subagent returns.
   - Run `.github/scripts/checkpoint.sh complete <STORY_ID> spec_finalizer`.
3. **Create implementation plan** — Dispatch a subagent:
   - **Prompt**: "Read and follow `.github/skills/implementation-planner/SKILL.md`. Story ID: `<STORY_ID>`. Produce `.aforge/specs/<STORY_ID>/blueprint.md`."
   - **Description**: "Architect: implementation-planner"
   - **Model**: Resolve complexity `heavy` via `.github/skills/shared/model-strategy.md`
   - Verify output file exists after subagent returns.
   - Run `.github/scripts/checkpoint.sh complete <STORY_ID> blueprint`.
...
1. **Compile test spec** — Dispatch a subagent:
   - **Prompt**: "Read and follow `.github/skills/test-spec-compiler/SKILL.md`. Story ID: `<STORY_ID>`. Produce `.aforge/specs/<STORY_ID>/test-plan.md`."
   - **Description**: "TestGen: test-spec-compiler"
   - **Model**: Resolve complexity `medium` via `.github/skills/shared/model-strategy.md`
   - Verify output file exists after subagent returns.
   - Run `.github/scripts/checkpoint.sh complete <STORY_ID> test_plan`.
2. **Output**: `[TestGen] Complete: <N> test cases`

### Stage 3: Code

1. **Read the dispatch protocol**: Read `.github/skills/subagent-task-dispatcher/SKILL.md` for the full dispatch workflow.
2. **Read the model strategy**: Read `.github/skills/shared/model-strategy.md` for complexity-to-model resolution.
...
4. **For each task** (in order), follow the Subagent Task Dispatcher protocol (Steps 1–7 in that skill). Pass these parameters:
   ...
   - After each task completes, run `.github/scripts/checkpoint.sh complete <STORY_ID> task_<N>` (e.g., `task_1`, `task_2`).
4. **Output**: `[Code] Complete: <N> files changed, <M> tests passing`

### Stage 4: Closure

1. **Build PR description** — Dispatch a subagent:
   - **Prompt**: "Read and follow `.github/skills/pull-request-builder/SKILL.md`. Story ID: `<STORY_ID>`. Produce `.aforge/specs/<STORY_ID>/PR_DESCRIPTION.md`."
   - **Description**: "Closure: pull-request-builder"
   - **Model**: Resolve complexity `light` via `.github/skills/shared/model-strategy.md`
   - Verify output file exists after subagent returns.
2. **Create PR**: Run `.github/scripts/pr.sh <STORY_ID>`.
3. **Mark complete**: Run `.github/scripts/checkpoint.sh complete <STORY_ID> pr`.

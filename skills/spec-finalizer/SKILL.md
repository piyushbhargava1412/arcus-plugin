...
### Step 0: Setup
Identify the skills folder (typically `.github/skills/` or `.agent/skills/` or `.claude/skills`). Store this in an internal variable called `skills_folder`. In the below steps, resolve `${skills_folder}` to the actual identified path.
...
Consult `${skills_folder}/spec-finalizer/references/decision-heuristics.md` for resolution strategies and option generation rules.
...
For each ambiguity, select the best option using the priority order defined in `${skills_folder}/spec-finalizer/references/decision-heuristics.md`.
...
Write the decisions to `.aforge/specs/[STORY-ID]/assumptions.md` using the template at `${skills_folder}/spec-finalizer/assets/assumptions-template.md`.
...
- **Assumptions Template**: `${skills_folder}/spec-finalizer/assets/assumptions-template.md`
- **Decision Heuristics**: `${skills_folder}/spec-finalizer/references/decision-heuristics.md`

...
- Format the branch name as `agent-forge/[STORY-ID]`.

### Step 2: Workspace Scaffolding
- Create the specification directory: `.aforge/specs/[STORY-ID]/`.
- If the full story text was provided, write it to `.aforge/specs/[STORY-ID]/story.md`.
...
- Create and switch to the new feature branch:
  `git checkout -b agent-forge/[STORY-ID]`
- If the branch already exists, switch to it: `git checkout agent-forge/[STORY-ID]`.
...
- **Bootstrapped**: The `.aforge/specs/` folder is ready for the `context-pack-builder`.

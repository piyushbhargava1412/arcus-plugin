...
| **L1-4** | Advisory reviewers are read-only (category invariant): `user-invocable: false`, a `tools:` allowlist naming no write-capable tool (the half hosts actually enforce), and `disallowed-tools: [Edit, Write, MultiEdit]` as defence-in-depth | Hard fail | `tests/fixtures/write-enabled-reviewer/SKILL.md` |

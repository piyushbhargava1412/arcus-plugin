...
#              .aforge/specs/<STORY_ID>/ workspace. Auto-increments branch index.
...
if [ -z "$BASE_BRANCH" ]; then
    BASE_BRANCH="${AFK_BASE_BRANCH:-$(git rev-parse --abbrev-ref HEAD)}"
fi
...
while IFS= read -r ref; do
    ...
    } | sort -u | grep -E "^agent-forge/${STORY_ID}-[0-9]+$" || true
)
BRANCH_NAME="agent-forge/${STORY_ID}-$(( max_run + 1 ))"
...
# Ensure .aforge is in .gitignore
if ! grep -q "^\.aforge" .gitignore 2>/dev/null; then
